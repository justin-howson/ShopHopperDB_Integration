...
const Sfox_url = "https://www.stonefoxclothing.com/shop-all?page=100";
...
async function scrapeMain(url,page) {
    ...
    result = $("li[data-hook='product-list-grid-item']").map((index,element) =>{
        ...
        let business_name = $("title").text().split('|')[1].trim();

        if(business_name =="Women’s Pants For Sale" )
        {
...
async function scrapeSecondary(item,page)
{
    ...
    for(var i=0;i<item.length;i++){
        ...
        let json_pindex = 'productPage_CAD_' + item[i].url.split('/')[4];
        let is_size_or_color = $("div[data-hook='product-options-inputs'] > div.cell:nth(0)").text();
      
        let data = $('script[id = "wix-warmup-data"]').html();
        data = JSON.parse(data);
        console.log(data);
       
        product = data.appsWarmupData['1380b703-ce81-ff05-f115-39571d94dfcd'][json_pindex].catalog.product;
        let v = product.productItems;
        let img = product.media;
...
export async function main()
{
    ...
    await writeJSOn("backend/WixScrapers/wixData.json",data_s);

    let data = cleaned_info;
    return data;
